...
type FormulaKey = 'gr' | 'ml' | 'lurr' | 'amr';
...
const FORMULA_KEYS: FormulaKey[] = ['gr', 'ml', 'lurr', 'amr'];
...
function computeGR(_pga: number): FormulaResult {
...
function computeML(pga: number): FormulaResult {
...
function combine(gr: FormulaResult, ml: FormulaResult, lurr: FormulaResult, amr: FormulaResult): Combined {
  const W: Record<FormulaKey, number> = { gr: 0.40, ml: 0.30, lurr: 0.20, amr: 0.10 };
  const R: Record<FormulaKey, FormulaResult> = { gr, ml, lurr, amr };
  const prob   = Math.round(FORMULA_KEYS.reduce((s, k) => s + R[k].probability * W[k], 0));
  const mag    = parseFloat(FORMULA_KEYS.reduce((s, k) => s + R[k].magnitude * W[k], 0).toFixed(1));
  const magMin = parseFloat(Math.min(...FORMULA_KEYS.map((k) => R[k].magnitude)).toFixed(1));
  const magMax = parseFloat(Math.max(...FORMULA_KEYS.map((k) => R[k].magnitude)).toFixed(1));
  ...
  const risk: Combined['risk']  = prob > 60 ? 'high' : prob > 35 ? 'moderate' : 'low';
  return { prob, mag, magMin, magMax, tMin, tMax, reliability, precision, agreement, risk };
}

// ─── Helpers ──────────────────────────────────────────────────────────────────
const RC   = (r: string) => r === 'high' ? '#ff4757' : r === 'moderate' ? '#ffa502' : '#2ed573';
const SC   = (v: number) => v >= 75 ? '#2ed573' : v >= 50 ? '#ffa502' : '#ff4757';
const SCL  = (v: number) => v >= 75 ? 'High' : v >= 50 ? 'Moderate' : 'Low';

// ─── Score gauge (arc) ───────────────────────────────────────────────────────
function Gauge({ label, value, Icon }: { label: string; value: number; Icon: LucideIcon }) {
  const c = SC(value);
  return (
...
// ─── Formula card ─────────────────────────────────────────────────────────────
function FormulaCard({ id, result }: { id: FormulaKey; result: FormulaResult }) {
...
export default function PredictionPage({
  ...
  const [results, setResults] = useState<Record<FormulaKey, FormulaResult | null>>({ gr:null, ml:null, lurr:null, amr:null });
  const [comb,    setComb]    = useState<Combined | null>(null);
  ...
  useEffect(() => {
    runAll();
    // eslint-disable-next-line react-hooks/exhaustive-deps -- mount-only bootstrap like CRA original
  }, []);

  const rc = comb ? RC(comb.risk) : 'var(--teal)';

  const compareData = FORMULA_KEYS.map((id) => ({
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:16 }}>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'1fr 370px', gap:16, alignItems:'start' }}>
            ...
            <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
              ...
              {FORMULA_KEYS.map((id) => {
                const r = results[id];
                return r ? <FormulaCard key={id} id={id} result={r} /> : null;
              })}
