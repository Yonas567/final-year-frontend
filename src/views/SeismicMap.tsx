...
type EthiopiaSensor = (typeof ETHIOPIA_SENSORS)[number];

interface LocalMapEvent {
  id: number;
  time: string;
  magnitude: number;
  depth: number;
...
const ZONE_DANGER_MAP = ZONE_DANGER as Record<string, { score: number; label: string; color: string; bg: string }>;

const EMPTY_PREDICTION: StationPrediction = {
  probability: 0,
  magnitude: 0,
  confidence: 0,
  risk: 'low',
...
function genPred(s: (typeof ETHIOPIA_SENSORS)[number]): StationPrediction {
...
export default function SeismicMapPage() {
  const [sel,      setSel]      = useState<EthiopiaSensor | null>(null);
  const [live,     setLive]     = useState<Record<string, number>>({});
  const [waves,    setWaves]    = useState<Record<string, number[]>>({});
  const [evts,     setEvts]     = useState<Record<string, LocalMapEvent[]>>({});
  ...
  useEffect(()=>{
    ...
    const e: Record<string, LocalMapEvent[]> = {};
    const p: Record<string, StationPrediction> = {};
  ...
  const sPred: StationPrediction = sel ? (preds[sel.id] ?? EMPTY_PREDICTION) : EMPTY_PREDICTION;
  const sWave= sel ? (waves[sel.id]||[]) : [];
  const sEvts= sel ? (evts[sel.id]||[]) : [];
  const wData= sWave.map((v: number, i: number)=>({i,v}));
  const radarData= sel ? [
          ...
          {!sel ? (
            <div style={{background:'var(--surface)',border:'1px solid var(--border)',borderRadius:'var(--radius-lg)',padding:'32px 20px',textAlign:'center'}}>
              ...
              <div style={{marginTop:16,display:'flex',flexDirection:'column',gap:5,textAlign:'left'}}>
                ...
                {ETHIOPIA_SENSORS.sort((a,b)=>(ZONE_DANGER_MAP[b.zone]?.score||0)-(ZONE_DANGER_MAP[a.zone]?.score||0)).slice(0,5).map(s=>{ const pga=live[s.id]||s.pga; const color=s.status==='offline'?'#5a6e8a':ZONE_COLOR[s.zone];
                ...
                <div style={{display:'flex',flexDirection:'column',gap:10}}>
                  <Panel title="Live Waveform">
                    <ResponsiveContainer width="100%" height={110}>
                      <AreaChart data={wData} margin={{top:4,right:4,left:-28,bottom:0}}>
                        ...
                        <Tooltip contentStyle={{background:'#162035',border:'1px solid #1e2d45',borderRadius:8,fontSize:10}} formatter={(v) => [`${Number(v).toFixed(4)}g`,'PGA']} labelFormatter={()=>''}/>
                  ...
                  <Panel title="Hourly PGA — Last 24h">
                    <ResponsiveContainer width="100%" height={110}>
                      <BarChart data={hourly} margin={{top:4,right:4,left:-24,bottom:0}}>
                        ...
                        <Tooltip contentStyle={{background:'#162035',border:'1px solid #1e2d45',borderRadius:8,fontSize:10}} formatter={(v) => [`${Number(v).toFixed(4)}g`,'PGA']}/>
                  ...
                  <Panel title="Recent Events — This Area">
                    <div style={{display:'flex',flexDirection:'column',gap:5}}>
                      {sEvts.slice(0,6).map((ev: LocalMapEvent)=>(
              ...
              {tab==='analysis' && (
                <div style={{display:'flex',flexDirection:'column',gap:10}}>
                  <Panel title="Zone Risk Scores">
                    {[{label:'Seismic Danger',v:ZONE_DANGER_MAP[sel.zone]?.score||15,color:sc},{label:'Alert Rate (30d)',v:Math.min(100,sel.alerts*3),color:'var(--amber)'},{label:'Avg PGA Level',v:Math.min(100,(sel.pga/0.2)*100),color:'var(--teal)'},{label:'Historical Activity',v:({critical:88,high:62,moderate:38,low:18} as Record<string, number>)[sel.zone]||18,color:'var(--green)'}].map(({label,v,color})=>(
